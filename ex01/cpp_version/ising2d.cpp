...
//const int tot_N = Nthermalization + Nsample*Nsubsweep; //total number of generated samples
//int rand_int[2*tot_N];
//double rand_dbl[tot_N];
...
int nn_sum(const Eigen::MatrixXi& x, const int i, const int j){
    ...
    return s*(x((i+1)%L, j) + x((i-1+L)%L, j) + x(i, (j+1)%L) + x(i, (j-1+L)%L));  
}
...
*/
void move(Eigen::MatrixXi& x, double& M, int& E, const Eigen::VectorXd& R_vec, const int& n_){
...
int main(int argc, char *argv[]){
    ...
    //fill array with precomputed random numbers
    /*
    for(int i = 0; i < tot_N; ++i){
        rand_int[2*i]   = uniform(gen);
        rand_int[2*i+1] = uniform(gen);
        rand_dbl[i]     = uniform_real(gen);
    }
    */
    ...
    Eigen::MatrixXi x = Eigen::MatrixXi::Random(L,L);
    ...
    double M = double(x.sum());
    int E = total_energy(x);
    ...
    M_data(0) = std::abs(M)*N_inv;
    E_data(0) = double(E);

    for(int n = 1; n < Nsample; ++n){
        for(int N_ = 0; N_ < Nsubsweep; ++N_){
            move(x, M, E, R_vec, Nthermalization + n*Nsubsweep + N_);
        }
        M_data(n) = std::abs(M)*N_inv;
        E_data(n) = double(E);
    }
    ...
    std::cout << temp << "," << M_mean << "," << M_std << "," << N*beta*M_std << "," << E_mean << "," << E_std << "," << beta*beta*E_std << "\n";
