...
using Wolff_type = float;
using SwendsenWang_type = int;
...
public:
  ...
  /* Runs a benchmark at specified temperature.
   * First thermalizes in thermalize_steps updates, then performs and times num_samples updates; sample_stride is ignored.
   * Returns std::pair {double: runtime in seconds, unsigned long long: total number of sites updated}
   * This can be used to measure runtime, and to figure out the #updates-per-sweep relation.
  */
  template <typename T>
  void run_benchmark();
  ...
  /* Save the data taken in the previous benchmark as comma separated values.
   * The data is best analyzed using pandas.
   * filename -- File to which the data is saved.
  */
  void save_benchmark_data(std::string filename) const;
...
private:
  ...
  template<typename T>
  int step_(int num=1); //returns number of updated sites
  template<typename T>
  void fetch_sample_();
  void extract_data_();
  ...
  //swendsen wang helpers
  void sw_reset_clusters_();
  int sw_find_(int x); //also chops the trees
  void sw_union_(int x, int y);

  //System Parameters
  const unsigned N_; //linear system size
  const double J_; //coupling
  std::vector<short int> spins_; //sites
  std::vector<double> exp_table_; //exp table
  double cluster_growth_probability_; //probability to occupy bond in cluster algorithms
  //std::mt19937_64 rng_; //random number generator slow
  std::minstd_rand0 rng_; //random number generator fast
  ...
  std::vector<double> qt_magnetizations_; //quartic magnetizations

  //Cluster Algorithm Parameters
  //Wolff
  //TODO: An unordered container could allow for vectorization (with random number buffer)
  std::vector<std::array<int,3> > cl_wolff_queue_; //Sites to be considered for addition in Wolff

  //Swendsen Wang
  std::vector<int> sw_label_; //assigned cluster
  std::vector<int> sw_labels_; //cluster connections
  std::vector<int> sw_flipper_; //multiplier for cluster i spins
  ...
  std::vector<std::vector<double> > corr_linear_energies_;
  std::vector<std::vector<double> > corr_linear_magnetizations_;
  std::vector<short int> corr_t0_configuration_;
  double corr_t0_magnetization_;
  //Benchmark Data
  std::vector<std::tuple<std::string, unsigned, unsigned, double, double, unsigned long long, unsigned> > bench_data_; //{method, dimension, linear system size, temperature, runtime[ms], #sites updated, #steps}
};
